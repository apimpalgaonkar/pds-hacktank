...
//CreateTarget creates the deployment target
func (dt *DeploymentTarget) CreateTarget(tenantID string, clusterID string, name string) (*pds.ModelsDeploymentTarget, *status.Response, error) {
	dtClient := dt.apiClient.DeploymentTargetsApi
	ctx, err := pdsutils.GetContext()
	if err != nil {
		log.Errorf("Error in getting context for api call: %v\n", err)
	...
	createTargetClusterBody := pds.ControllersCreateTenantDeploymentTarget{ClusterId: &clusterID, Name: &name}
	dtModel, res, err := dtClient.ApiTenantsIdDeploymentTargetsPost(ctx, tenantID).Body(createTargetClusterBody).Execute()
	if err != nil && res.StatusCode != status.StatusOK {
		log.Errorf("Error when calling `ApiDeploymentTargetsIdPut``: %v\n", err)
		log.Errorf("Full HTTP response: %v\n", res)
		return nil, nil, err
